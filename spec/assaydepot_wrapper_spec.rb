...
#      access_token = client.client_credentials.get_token(:redirect_uri => 'http://localhost:4567/oauth2/callback')
      AssayDepot.configure do |config|
        config.access_token = "ytvbsg4FNrG7q-bDH9pA"
        config.url = "#{site}/api/v1" unless ENV['LEGACY']
        config.url = "#{site}/api" if ENV['LEGACY']
      end
    end
    ...
    unless ENV['LEGACY']
      context "info" do 
        ...
        it "info version V1" do
          info["api_version"].should == "V1"        
        end
      end
    end
    ...
    context "quote groups" do
      ...
      it "quote groups specific id" do
        ...
        response["result"]["id"].should == id
      end
      
      it "quote group update is deprecated" do
        id = qg[AssayDepot::QuoteGroup.ref_name].first["id"]
        response = AssayDepot::QuoteGroup.post( {} )
      ...
      it "quote group add note" do
        ...
        unless ENV['LEGACY']
          response["code"].should == "server_error"
        else
          response["code"].should == nil
        end          
      end
    end
    ...
    context "quoted wares" do
      ...
      it "get all quoted wares" do
        qw[AssayDepot::QuotedWare.ref_name].is_a?(Array).should == true
      end
    end
    ...
    context "wares providers" do
      ...
      it "get posted provider for ware" do
        @ware_provider[AssayDepot::WareProvider.ref_name].select { |el| 
          el["reference_of_id"] == @provider_id 
        }.length.should == 1
      end        
